...
Window {
    ...
    highlight: Component{Rectangle{z: 1; height: 0; color: 'green'}} highlightMoveDuration: 200; model: Emojis.emojis.filter((i) => i.emojis[1].description.includes(listview.headerItem.children[0].text));
    delegate: Rectangle{ color: "#f0f0f0"; width: tabview.width/listview.model.length ; height: 40; Text{anchors.centerIn: parent; text: modelData.emojis[1].emoji; font.pointSize: 16}
    ...
    ListView{id: listview; anchors.top: tabview.bottom; anchors.bottom: parent.bottom; width: parent.width;
        anchors.bottomMargin: 100; topMargin: 10; spacing: 70;
        ...
        header: Component{ Rectangle{ width: parent.width; height: 30; z: 2; radius: 12;
                ...
                onTextChanged: { console.log(listview.headerItem.children[0].text) }}}}
        headerPositioning: listview.headerItem.children[0].text !== "" ?  listview.PullBackHeader : listview.OverlayHeader
        model: Emojis.emojis.filter((i) => i.emojis[1].description.includes(listview.headerItem.children[0].text));
